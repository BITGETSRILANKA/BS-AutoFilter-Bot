...
from firebase_admin importimport os
import json
import math
import logging
import asyncio
...
async def post_movie_to_channel(file_data, user_id):
    ...
    try:
        ...
        if MOVIE_CHANNEL.startswith('@'):
            channel_username = MOVIE_CHANNEL.lstrip('@')
            message_link = f"https://t.me/{channel_username}/{message.id}"
        elif isinstance(MOVIE_CHANNEL, str) and MOVIE_CHANNEL.isdigit():
            # For private channels with negative ID
            message_link = f"https://t.me/c/{MOVIE_CHANNEL[4:] if MOVIE_CHANNEL.startswith('-100') else MOVIE_CHANNEL}/{message.id}"
        else:
            message_link = f"https://t.me/c/{str(MOVIE_CHANNEL)[4:]}/{message.id}"
        ...
        task_key = f"{message.id}_{MOVIE_CHANNEL}"
        delete_task = asyncio.create_task(
            delete_message_after_delay(message.id, MOVIE_CHANNEL, 2, "channel_movie")
        )
        DELETE_TASKS[task_key] = delete_task
